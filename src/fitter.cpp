...
YawRange computeYawRange(const geo::Pose3D& sensor_pose_xya, const geo::Pose3D& expected_entity_pose,
...
Fitter::Fitter(unsigned int nr_data_points, float fx) : nr_data_points_(nr_data_points)
{
...
bool Fitter::evaluateCandidate(const EstimationInputData& static_data, Candidate& candidate) const
{
  ...
  double ds =
      static_data.sensor_ranges[candidate.beam_index];      // Transformed sensor reading (distance measured along beam)
  ...
  candidate.pose.t += candidate.beam_direction *
                      ((ds - dm) * candidate.beam_length);  // JL: Why multiply with beam_length (or, at least, 'l')?
                                            ...
                                            const geo::Pose3D& sensor_pose_xya, const int expected_center_beam) const
{
  ...
  if (expected_center_beam < 0 || expected_center_beam >= static_cast<int>(nr_data_points_))
    throw FitterError("Expected beam outside of measurement range(" + std::to_string(nr_data_points_) +
                      "), index: " + std::to_string(expected_center_beam));
...
{
  unsigned int nr_beams =
      std::min(200, cammodel.fullResolution().width);  // don't use more data points than the resolution of your camera
  double fx = cammodel.fx() * nr_beams /
              cammodel.fullResolution().width;  // Reducing nr of data points will require a different focal length
