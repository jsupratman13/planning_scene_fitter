                  ...
                  std::vector<geo::Vec2i>& line_starts, cv::Mat& contour_map, bool add_first)
{
  ...
  while (true)
  {
    ...
    for (int i = -1; i < 3; ++i)
    {
      ...
      {
        ROS_ERROR("This should not happen! Image going out of bound, findContours. [%d, %d] and image size is [%d,%d]",
                  idx_y, idx_x, image.rows, image.cols);
        return;
      }
    ...
    {
      ROS_ERROR("This should not happen! Contour map going out of bound, findContours. [%d, %d] and contour map size "
                "is [%d,%d]",
                p_current.y, p_current.x, contour_map.rows, contour_map.cols);
      return;
    }
...
void project2D(const geo::Mesh& mesh, std::vector<std::vector<geo::Vec2> >& contours)
{
  ...
  //    for(std::vector<std::vector<geo::Vec2i> >::const_iterator it = contours_pixel.begin(); it !=
  //    contours_pixel.end(); ++it)
  //    {
